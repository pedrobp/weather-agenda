...
import { addDays, addMonths, format, isSameDay, isWeekend, startOfWeek } from 'date-fns'
import { IconButton, makeStyles, Typography, Snackbar, Tooltip, Button } from '@material-ui/core'
...
import { selectSnackBar, hide as hideSnackbar } from '../reducers/snackbarSlice'
...
const Calendar = () => {
  ...
  return (
    <div className={classes.root}>
          ...
          {dateMatrix?.map((week, weekIndex) => (
            <div key={weekIndex} className={classes.weekRow}>
              {week.map((day, dayIndex) => (
                <div key={dayIndex} className={clsx(classes.dayContainer, classes.border, isWeekend(day) ? classes.weekendContainer : '')}>
                  <div className={classes.dayHeader} style={{ '&:hover > *': { display: 'block' } }}>
                    <Typography className={classes.dayOfMonth} style={{ color: getDayColor(day) }}>
                      {day.getDate()}
                    </Typography>
                    <Tooltip arrow enterDelay={300} title="Add a new reminder">
                      <IconButton id="icon" className={classes.addIcon} onClick={() => handleOpenDialog(day)}>
                        <Add />
                      </IconButton>
                    </Tooltip>
                  </div>
                  <div className={classes.chipList}>
                    {reminders
                      ...
                      .map((r, chipIndex) => (
                        <ReminderChip key={chipIndex} data={r} handleOpenReminder={() => handleOpenReminder(r)} />
                      ))}
                  </div>
                </div>
              ))}
      ...
      <Button color="primary" variant="contained" className={classes.addButton} onClick={() => handleOpenDialog(new Date())}>
        <AddIcon />
        New Reminder
      </Button>
    </div>
