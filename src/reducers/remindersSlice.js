...
export const remindersSlice = createSlice({
  ...
  reducers: {
    ...
    editReminder: (state, action) => {
      const currentReminder = state.value.find((r) => r.id === action.payload.id)
      const index = state.value.indexOf(currentReminder)

      state.value = [...state.value.slice(0, index), action.payload, ...state.value.slice(index + 1, state.value.length + 1)]
    }
  }
})
